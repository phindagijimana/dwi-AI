...
Targets the Gugger_Lab dwi_test2 pipeline output:
    ...
    <root>/sub-XXX/dk_nodes.mif          label image (required for --with-volume-ai)
...
* Per-pair side_ai = (L-R)/(L+R) and log_ai = ln(L/R) for matched L/R DK ROIs.
* Optionally (--with-volume-ai): ROI volume from ``dk_nodes.mif`` and the same
  interhemispheric AI formulas on volume.
"""
